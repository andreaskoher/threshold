...
# ###################################################################################################
# 
#                         Individual-based model unweighted!
#
# ###################################################################################################
...
cpdef mydouble psr2uw(mydouble ladda, mydouble mu, int [:] lip, int [:] li, mydouble [:] ld, int [:] lp, myint N, myint T, myint valumax, mydouble tolerance, myint store, mydouble sr_target):
    ...
    if flag_above_one_warning:
        warnings.warn("Value error: vector element was larger than one")
    ...
    with nogil:
        ...
        while flag != 1 and k < T*valumax:
            ...
            while i<N:
                ...
                # diagonal
                if diag_element > 0:
                    v2[i] = pow(laddam1, diag_element) * (1. - mu) * v[i]
                elif diag_element == 0:
                    v2[i] = (1. - mu) * v[i]
